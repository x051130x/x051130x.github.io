...
如提出的问题已在上述提到的文档中有相关叙述，管理员有权**不对您的问题作出回应**。

3. 如以上内容都无法解决问题，请考虑在群内提问。提问时务必清楚描述您的问题。如果有，请务必附带上日志/崩溃报告/与问题相关的完整且清晰的屏幕截图等材料。
...
### 获取客户端包体
  ...
  整合包部分子服与养老部分子服客户端不通用，分别打包为两个文件。  
    ==> 了解子服务器之间的差异：
...
### 运行 ServerSync
  ...
  仅养老部分子服客户端需要进行此步骤，整合包客户端可直接运行。  

  养老子服客户端使用 ServerSync。这个软件可以帮助你获得最新的 Mods 和 FTB Quests 文件等。  
  通过客户端根目录下的 sync.cmd 运行。  
...
### sync 无法运行（环境变量相关）
  ...
  运行 sync 时得到`'java'不是内部或外部命令，也不是可运行的程序或批处理文件。`的输出，此时 sync 因没有配置环境变量而无法运行。转到情况1  

  运行后得到类似`无法连接到服务器`的输出，转到情况2
...
确定设备网络连接情况正常，则 sync 服务端可能更换了连接地址。  
查阅群公告是否有相关说明，或向腐竹确认索要新的配置文件。
